...
//        checks if m is zero,
//        if not then uses another NN to hold the value of n mod m
//        recursion to reduce to GCD
        if (!m.isZero()) {
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        NaturalNumber t = new NaturalNumber2(2);

//        checks if n is even, return bool
        if (n2.divide(t).isZero()) {
            evenNum = true;
        }
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        NaturalNumber o = new NaturalNumber2(1);
        NaturalNumber t = new NaturalNumber2(2);
        NaturalNumber p2 = new NaturalNumber2(p);
        NaturalNumber nSave = new NaturalNumber2(n);
...
//         initializing vars & setting vals
        NaturalNumber o1 = new NaturalNumber2(1);
        NaturalNumber o2 = new NaturalNumber2(1);
        NaturalNumber o3 = new NaturalNumber2();
        NaturalNumber o4 = new NaturalNumber2();

        int temp = 0;
        NaturalNumber wSave = new NaturalNumber2(w);
...
//         check if w is a witness to compositeness
        boolean comp = false;
        if (o1.equals(new NaturalNumber2(1))
                || !o2.equals(new NaturalNumber2(1))) {
            comp = true;
        }
        return comp;
    }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            for (int i = 1; i < 50; i++) {
                NaturalNumber randGen = randomNumber(n2);

                while (randGen.compareTo(o) <= 0
                        || randGen.compareTo(n2) >= 0) {
                    randGen = randomNumber(n2);
                }

                // check if composite
                if (isWitnessToCompositeness(randGen, n)) {
                    isPrime = false;
                }
            }
        }
...
//        if even, then increment to odd num (only odd can be prime except 2)
        if (isEven(n)) {
            n.increment();
        }
